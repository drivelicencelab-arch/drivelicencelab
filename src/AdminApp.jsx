...
import { ReportExportButton } from './services/ReportExport.js'
const BulkUpload = lazy(() => import('./services/BulkUpload.jsx'))
  ...
  return (
    <div style={{ fontFamily: "'Space Grotesk',sans-serif", background: T.bg, minHeight: '100vh', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '20px 16px' }}>
        ...
        {tab === 'bulk' && (<>
          <TopBar title="Bulk Upload" subtitle="Import multiple students at once" />
          <Suspense fallback={<div style={{textAlign:'center',padding:32,color:'#8B949E'}}>Loading uploader…</div>}>
            <BulkUpload
              schoolId={schoolId}
              slots={slots}
              onDone={() => loadEnrollments(schoolId)}
            />
          </Suspense>
        </>)}

        {tab === 'reports' && (<>
          <TopBar title="Performance Reports" subtitle="Download student data as spreadsheets" />
          <GlassCard style={{ marginBottom: 16 }} glow={T.green}>
            <div style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, color: T.text, marginBottom: 8, fontSize: 16 }}>📊 Full School Report</div>
            <div style={{ color: T.textSub, fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>
              Downloads 3 CSV files:<br/>
              • Student profiles + enrolment details<br/>
              • Quiz scores + performance history<br/>
              • Attendance records
            </div>
            <ReportExportButton schoolId={schoolId} schoolName="DriveLicenceLab School" />
          </GlassCard>

          <GlassCard glow={T.teal}>
            <div style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, color: T.text, marginBottom: 8, fontSize: 16 }}>📈 School Summary</div>
            <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
              {[
                { l: 'Total Students', v: totalEnrolled, c: T.teal, i: '👥' },
                { l: 'Instructors', v: instructors.length, c: T.green, i: '👨‍🏫' },
                { l: 'Active Slots', v: activeSlots, c: T.orange, i: '⏰' },
              ].map((s, i) => (
                <div key={i} style={{ flex: 1, textAlign: 'center', background: s.c+'15', border: `1px solid ${s.c}44`, borderRadius: 12, padding: 12 }}>
...
// Bulk upload and reports tabs are appended here via AdminAppExtension
